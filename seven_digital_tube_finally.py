...
def seven_digital_tube(x, y, number, m, n, k) :
    ...
    for _i in d_num :
        ...
        one_d_tube = one_digital_tube(d_t_pos[0](x), d_t_pos[1](y), d_t_pos[2], d_t_pos[3], d_t_pos[4], (199, 199, 199))
...
def calculate_table_size(m, n, k, p) :
    ...
    date_table_in_x_y = []
    for i in range(1,5) :
        date_table_in_x_y.append((date_table_x + (6/m + 4.99)*p, date_table_y - p - i*d_width/4))
    # 时间表盘起始点(x, y)
...
def draw_clock(m, n, k, p):
    ...
    tabler.hideturtle()
    tabler.pu()
    # 绘制外表盘，四周倒圆角,圆角的半径定为10，线框的线宽8(n=10)
    draw_table(c_t_size[0][0] + n, c_t_size[0][1], c_t_size[0][2] - 2*n, c_t_size[0][3] - 2*n, n, 4*n/5, (232, 232, 232))
    # 绘制日期表盘，四周倒圆角,圆角的半径定为10，线框的线宽5
    draw_table(c_t_size[1][0] + n, c_t_size[1][1], c_t_size[1][2] - 2*n, c_t_size[1][3] -2*n , n, n/5, (232, 232, 232))
    # 绘制时间表盘，四周倒圆角,圆角的半径定为10，线框的线宽5
    draw_table(c_t_size[2][0] + n, c_t_size[2][1], c_t_size[2][2] - 2*n, c_t_size[2][3] -2*n , n, n/5, (232, 232, 232))
...
n = m/6  # 单个数码管的宽度
k = m/12   # 单个数码管之间的间隔
p = m/2  # 完整数码管之间的间隔,表盘之间的间隔

# 初始化turtle
t.setup((m + 2*1.414*n + 2*k)*7 + 14*p +100, 2*m + 3*1.414*n + 4*k + 4*p + 100)
t.hideturtle()
t.colormode(255)
